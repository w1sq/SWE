...
func NewFileStorageServer() *FileStorageServer {
	// Store files in ./data directory (ensure it exists)
	const dir = "./data"
...
func (s *FileStorageServer) StoreFile(ctx context.Context, req *pb.StoreFileRequest) (*pb.StoreFileResponse, error) {
	fileID := generateFileID() // You can use a UUID or similar
	filePath := filepath.Join(s.storageDir, fileID)
	err := os.WriteFile(filePath, req.Content, 0644)
	if err != nil {
		return nil, fmt.Errorf("failed to store file: %w", err)
	}
	// Optionally, store filename mapping (e.g., in a simple map or DB)
...
// generateFileID returns a unique file ID (use a better method in production)
